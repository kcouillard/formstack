# On-The-Job Project for Formstack by Kerri Couillard
...
Advanced Search Form (calls out to Escapia software via API to get properties that match the inputs)
Inquire Form (sends an inquiry about a property into the Escapia software via API)
Get Quote Form (requests price for a property based on inputs to the Escapia software via API)
Make Reservation Form (books a property based on inputs in the Escapia software via API)
...
This information could help them to determine exactly how to grow their inventory best for their customers.
...
Password: mghB7xDd
Your API Trial Key: WS72-SHR1-FOT4
